    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur border-b border-slate-200 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-slate-800">Flagged NCTs Game</h1>
          {pendingCount !== null && (
          ...
          {drugData['Generic Name'] && (
            <span className="ml-2 px-3 py-1 text-xs font-medium rounded-full bg-cyan-50 text-cyan-700 border border-cyan-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-0 h-[calc(100vh-57px)]">
        <div className="lg:col-span-3 overflow-y-auto p-5 space-y-4">
          ...
          <div className="rounded-xl border border-cyan-200 bg-gradient-to-br from-cyan-50 to-sky-50 p-5">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-2 h-2 rounded-full bg-cyan-500"></div>
              <h2 className="text-xs font-semibold text-cyan-700 uppercase tracking-wide">Flagged Drug Information</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
              {Object.entries(drugData).map(([key, value]) => (
                <div key={key} className={`flex flex-col ${key === 'NCTs' ? 'md:col-span-2' : ''}`}>
                  <span className="text-[11px] font-medium text-cyan-600 uppercase tracking-wide">{key}</span>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <input className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-cyan-200 focus:border-cyan-300 focus:outline-none transition" value={brandName} onChange={(e) => handleBrandChange(e.target.value)} placeholder="Enter Brand Name" />
          </div>

          {indications.map((indication, index) => (
            <div key={`indication-${index}`} className="rounded-xl border-2 border-cyan-200 bg-white p-5">
                    ...
                    <div key={`trial${trial_index}`} className="mb-2">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <textarea rows="2" className="w-full p-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-cyan-200 focus:outline-none" value={trial.sub_cancer} onChange={(e) => handleIndicationChange(index, trial_index, 'sub_cancer', e.target.value)} placeholder="Enter Sub Cancer" />
                        </div>
                        <div>
                          <div className="text-[10px] text-slate-400 mb-0.5">NCTs</div>
                          <textarea rows="2" className="w-full p-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-cyan-200 focus:outline-none" value={trial.ncts} onChange={(e) => handleIndicationChange(index, trial_index, 'ncts', e.target.value)} placeholder="Enter NCTs" />
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {READONLY_FIELDS.filter(f => indication[f.key]).map(field => (
                    <div key={field.key} className={field.key === 'description' || field.key === 'other' ? 'md:col-span-2' : ''}>
                      ...
                      <AutoResizeTextarea key={`${field.key}-${index}-${indication[field.key]}`} content={indication[field.key]} disabled={true} placeholder={field.label} />
                    </div>
                  ))}
                </div>
              </div>
